...
///
/// STRAIGHT PORT of Android `ui/components/WorldLandmask.kt` — the Base64 payload
/// below is byte-identical to the Kotlin one, so both platforms rasterise exactly
/// the same continents. Regenerate BOTH from `scripts/generate-landmask.js`
/// (Natural Earth 50m physical land vectors, public domain) or they will drift.
///
/// Storage: 720×360 bits → 32400 bytes → 43200-char Base64, decoded once into a
/// `[Bool]` on first access. Little-endian *within* each byte (bit 0 first),
/// matching the Kotlin unpack loop.
///
/// Only `WorldGlobeView` reads this; the coastline-polygon extras that Android
/// carries are NOT ported (nothing on iOS draws vector coastlines).
enum WorldLandmask {
    private static let cols = 720
    private static let rows = 360

    /// Decoded once, on first touch, off the render hot path. `.ignoreUnknownCharacters`
    /// is what lets the literal keep its newlines instead of being one 43k-char line.
    private static let cells: [Bool] = {
        guard let bytes = Data(base64Encoded: packedB64, options: .ignoreUnknownCharacters) else {
            // A corrupt literal must not crash the Connect screen — an all-ocean
            // globe is ugly but harmless, and the assert catches it in debug.
            assertionFailure("WorldLandmask base64 payload failed to decode")
    ...
    /// True if mask cell (r, c) is land. Columns wrap; rows out of range are sea.
